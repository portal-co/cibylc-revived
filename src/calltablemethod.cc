...
CallTableMethod::CallTableMethod(int maxFunctions,
...
void CallTableMethod::generateMethod(const char *name, int start, int end) {
  const char *ret_type = config->threadSafe ? "long" : "int";

  emit->generic("  public static final %s %s(int address, int sp, int a0, int "
                "a1, int a2, int a3) throws Exception {\n"
                "    %s ret = 0;\n"
                "    switch(address) {\n",
  ...
  for (int i = start; i < end; i++) {
    ...
    /* Pass registers */
    void *it;
    for (MIPS_register_t reg = mt->getFirstRegisterToPass(&it); reg != R_ZERO;
      ...
      if (reg == R_FNA) {
        ...
        panic_if(idx < 0,
                 "Could not find function index for function %s in method %s",
                 fn->getName(), mt->getName());
        emit->generic("%s %d", comma, idx);
      }
      if (reg == R_A0) {
        emit->generic("%s a0", comma);
        comma = ",";
      }
      if (reg == R_A1) {
        emit->generic("%s a1", comma);
        comma = ",";
      }
      if (reg == R_A2) {
        emit->generic("%s a2", comma);
        comma = ",";
      }
      if (reg == R_A3) {
  ...
  emit->generic("      default:\n"
                "         throw new Exception(\"Call to unknown location \" + "
...
void CallTableMethod::generateHierarchy(unsigned int n) {
  ...
  for (i = 0; i < n - 1; i++) {
    xsnprintf(buf, 80, "call%d", i);
    this->generateMethod(buf, i * functions_per_level,
                         (i + 1) * functions_per_level);
  }
  xsnprintf(buf, 80, "call%d", i);
...
bool CallTableMethod::pass2() {
  unsigned int functions_per_class =
      this->n_functions / config->callTableClasses;

  /* If it exists, generate a table of exported symbols */
  if (this->exp_syms) {
    emit->generic(" public static final int getAddressByName(String name) "
                  "throws Exception {\n");
  ...
  panic_if(config->callTableClasses != 1 && config->callTableHierarchy != 1,
           "Setting both number of call table classes (%d) and call table "
           "hierarchy (%d)\n"
           "is not allowed\n",
  ...
  if (config->callTableHierarchy > 1)
    ...
    if (config->callTableClasses > 1) {
      ...
      emit->generic("  public static final int call(int address, int sp, int "
                    "a0, int a1, int a2, int a3) throws Exception {\n");
      for (unsigned int i = 0; i < config->callTableClasses; i++) {
        ...
        if (i < config->callTableClasses - 1)
          emit->generic("    %s ( address >= 0x%08x && address < 0x%08x)\n"
                        "        return CibylCallTable%d.call(address, sp, a0, "
                        "a1, a2, a3);\n",
                        i == 0 ? "if" : "else if", first_fn->getAddress(),
                        last_fn->getAddress(), i);
        else /* The last */
          emit->generic("    else\n"
                        "        return CibylCallTable%d.call(address, sp, a0, "
                        "a1, a2, a3);\n",
    ...
    for (unsigned int i = 0; i < config->callTableClasses; i++) {
      ...
      if (config->callTableClasses > 1) {
        ...
        emit->setOutputFile(
            open_file_in_dir(controller->getDstDir(), buf, "w"));
