...
/* Class members */
JavaClass::JavaClass(const char *name, JavaMethod **in_methods, int first,
                     int last)
    : CodeBlock() {
  panic_if(last < first, "Creating a class with methods from %d to %d\n", first,
           last);
...
JavaMethod *JavaClass::getMethodByAddress(uint32_t addr, int *idx) {
  ...
  *idx = -1; /* Assume not found */
  ret = (JavaMethod **)bsearch(&tmp, this->methods,
                               this->n_methods - this->n_multiFunctionMethods,
...
const char *JavaClass::getName() { return this->name; }
...
bool JavaClass::pass2() {
  ...
  emit->generic(".class public %s%s\n"
                ...
                "\treturn\n"
                ".end method\n",
                controller->getJasminPackagePath(), this->getName());
...
CallTableClass::CallTableClass(const char *name, JavaMethod **methods,
                               int first, int last)
    : JavaClass(name, methods, first, last) {
...
bool CallTableClass::pass1() { return this->methods[0]->pass1(); }
...
JavaMethod *CallTableClass::getMethodByAddress(uint32_t addr) { return NULL; }
