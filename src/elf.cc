...
void CibylElf::handleSymtab(Elf_Scn *scn) {
  ...
  panic_if(n <= 0, "Section data too small (%zd) - no symbols\n", data->d_size);
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (type == STT_FUNC) {
      ElfSymbol *sym =
          new ElfSymbol(i, binding, s->st_value, s->st_size, type, sym_name);
      ...
      this->symbols[n_syms++] = this->dataSymbols[n_datas++] =
          new ElfSymbol(i, binding, (uint32_t)s->st_value, (uint32_t)s->st_size,
                        type, sym_name);
  ...
  panic_if(n_symbols <= 0, "No symbols in the ELF file (file stripped?)\n");
...
void CibylElf::handleDwarfFunction(Dwarf_Die *fun_die) {
  ...
  case AGGREGATE:
     ...
     *
     *   If the called function returns a structure or union, the caller passes
     * the address of an area that is large enough to hold the structure to the
     * function in $4. The called function copies the returned structure into
     * this area before it returns. This address becomes the first argument to
     * the function for the purposes of argument register allocation and all
     * user arguments are shifted down by one.
     */
    sym->ret_size = 1;
    sym->n_args++;
    break;
...
CibylElf::CibylElf(const char *filename) {
  ...
  panic_if(elf_version(EV_CURRENT) == EV_NONE, "ELF version failed on %s\n",
           filename);

  panic_if((fd = open(filename, O_RDONLY, 0)) < 0, "Cannot open %s\n",
           filename);

  panic_if(!(this->elf = elf_begin(fd, ELF_C_READ, NULL)),
           "elf_begin failed on %s\n", filename);

  panic_if(!(ehdr = elf32_getehdr(this->elf)), "elf32_getehdr failed on %s\n",
           filename);
  ...
  while ((scn = elf_nextscn(this->elf, scn)) != NULL) {
    ...
    panic_if(!data, "elf_getdata failed on section %s in %s\n", name, filename);

    this->addSection(new ElfSection(name, (uint8_t *)data->d_buf, data->d_size,
                                    shdr->sh_type, shdr->sh_addralign,
                                    shdr->sh_addr));
  ...
  panic_if(!this->getSection(".text") || !this->getSection(".cibylstrtab") ||
    ...
    if (shdr->sh_type == SHT_REL) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        if (sym && sym->type == STT_SECTION && type == R_MIPS_32) {
          ...
          ElfSection *sec =
              this->getSection(name + 4); // should use shdr->sh_link instead
          if (sec) {
  ...
  panic_if(!this->functionSymbols, "No function symbols in the ELF file\n");
  panic_if(!this->dataSymbols, "No data symbols in the ELF file\n");

  /* Sort the symbols and fixup addresses */
  this->fixupSymbolSize(this->functionSymbols, this->n_functionSymbols,
                        this->getEntryPoint() +
                            this->getSection(".text")->size);
  if (this->getSection(".data"))
    this->fixupSymbolSize(this->dataSymbols, this->n_dataSymbols,
...
ElfSymbol **CibylElf::getFunctions() { return this->functionSymbols; }

void CibylElf::addSection(ElfSection *section) {
  this->m_sectionsByName[section->name] = section;
}

int CibylElf::getNumberOfFunctions() { return this->n_functionSymbols; }

ElfReloc **CibylElf::getRelocations() { return this->relocs; }

int CibylElf::getNumberOfRelocations() { return this->n_relocs; }
