...
void CodeBlock::addJumptabLabel(uint32_t addr) {
  ...
  this->jumptabLabels = (uint32_t *)xrealloc(
      this->jumptabLabels, this->n_jumptabLabels * sizeof(uint32_t));
...
void CodeBlock::sortJumptabLabels() {
  qsort(this->jumptabLabels, this->n_jumptabLabels, sizeof(uint32_t),
        uint32_cmp);
}
