...
bool ExceptionHandler::pass2() {
  ...
  panic_if(!dstMethod, "No call table method???\n");
  dstClass = controller->getClassByMethodName(dstMethod->getName());
  panic_if(!dstClass, "No class for the call table method???\n");
...
JavaMethod::JavaMethod(Function **fns, int first, int last) : CodeBlock() {
  panic_if(first > last,
  ...
  this->m_possibleArguments =
      (MIPS_register_t *)xcalloc(7, sizeof(MIPS_register_t));
...
JavaMethod::~JavaMethod() { free(this->m_possibleArguments); }
...
bool JavaMethod::pass1() {
  ...
  for (MIPS_register_t reg = this->getFirstRegisterToPass(&it); reg != R_ZERO;
...
int JavaMethod::addReturnLocation(uint32_t address) {
  ...
  this->returnLocations = (uint32_t *)xrealloc(
      this->returnLocations, this->n_returnLocations * sizeof(uint32_t *));
...
void JavaMethod::emitSubroutineForOp(mips_opcode_t op) {
  panic_if(op != OP_LB && op != OP_LBU && op != OP_LH && op != OP_LHU &&
               op != OP_SB && op != OP_SH,
...
void JavaMethod::emitSubroutines() {
  ...
  for (int i = 0; i < this->n_functions; i++) {
    ...
    for (unsigned int j = 0; j < sizeof(ops_to_check) / sizeof(mips_opcode_t);
         j++) {
...
bool JavaMethod::pass2() {
  ...
  emit->generic("\n.method public static %s\n"
                ".limit stack %d\n"
                ".limit locals %d\n",
                this->getJavaMethodName(), this->getMaxStackHeight() + 2,
  ...
  for (int i = 0; i < N_REGS; i++) {
    panic_if(i == R_FNA && this->registerUsage[i] > 0 &&
                 !this->hasMultipleFunctions(),
  ...
  if (this->hasMultipleFunctions()) {
    uint32_t *table = (uint32_t *)xcalloc(this->n_functions, sizeof(uint32_t));
    ...
    emit->bc_tableswitch(0, this->n_functions, table,
...
void JavaMethod::setReturnSize(int n) {
  panic_if(n < 0 || n > 2, "return size is out of bounds for %s: %d\n",
...
char *JavaMethod::addExceptionHandler(ExceptionHandler *eh) {
  ...
  this->exceptionHandlers = (ExceptionHandler **)xrealloc(
      this->exceptionHandlers, (n + 1) * sizeof(ExceptionHandler *));
