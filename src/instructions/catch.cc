...
public:
  CatchInsn(uint32_t address, int32_t extra)
      : Instruction(address, 0, R_ZERO, R_ZERO, R_ZERO, extra) {
...
#define DELIMS " \t,\n"
    ...
    while (p) {
      ...
      this->exception_classes = (char **)xrealloc(
          this->exception_classes, sizeof(char *) * this->n_exception_classes);
...
#undef DELIMS
    ...
    this->handler =
        method->addExceptionHandler(new ExceptionHandler(start, end));
  ...
  size_t getMaxStackHeight() { return 2; }
