...
public:
  SyscallInsn(uint32_t address, int32_t extra)
      : Instruction(address, CIBYL_SYSCALL, R_ZERO, R_ZERO, R_ZERO, extra) {
    this->sysc = NULL;
  }
...
public:
  SyscallRegisterArgument(uint32_t address, int32_t extra)
      : Instruction(address, CIBYL_REGISTER_ARGUMENT, R_ZERO, R_ZERO, R_ZERO,
                    extra) {}
  ...
  size_t getMaxStackHeight() { return 1; }
};
