...
public:
  BranchInstruction(uint32_t address, int opcode, MIPS_register_t rs,
                    MIPS_register_t rt, MIPS_register_t rd, int32_t extra)
      : Instruction(address, opcode, rs, rt, rd, extra) {}
...
public:
  Jump(uint32_t address, int opcode, int32_t extra)
      : BranchInstruction(address, opcode, R_ZERO, R_ZERO, R_ZERO, extra) {}
...
public:
  Jal(uint32_t address, int opcode, int32_t extra)
      : BranchInstruction(address, opcode, R_ZERO, R_ZERO, R_ZERO, extra) {
    this->method = NULL;
    this->dstMethod = NULL;
    this->dstClass = NULL;
  ...
  Jal(uint32_t address, int opcode, MIPS_register_t rs, int32_t extra)
      : BranchInstruction(address, opcode, rs, R_ZERO, R_ZERO, extra) {
    this->method = NULL;
    this->dstMethod = NULL;
    this->dstClass = NULL;
    this->builtin = NULL;
  ...
  bool pass1() {
    ...
    this->dstClass =
        controller->getClassByMethodName(this->dstMethod->getName());
  ...
  virtual bool pass2() {
    ...
    if (this->opcode == OP_JAL && this->method->hasMultipleFunctions() &&
      ...
      panic_if(handle < 0, "Could not add return address at 0x%x: %d\n",
    ...
    for (MIPS_register_t reg = this->dstMethod->getFirstRegisterToPass(&it);
         reg != 0; reg = this->dstMethod->getNextRegisterToPass(&it)) {
    ...
    emit->bc_invokestatic("%s%s/%s", controller->getJasminPackagePath(),
                          dstClass->getName(),
                          this->dstMethod->getJavaMethodName());
  ...
  int fillDestinations(int *p) {
    ...
    out +=
        this->addToRegisterUsage(R_V0, p) + this->addToRegisterUsage(R_V1, p);
    if (this->builtin)
  ...
  int fillSources(int *p) {
    ...
    out +=
        this->addToRegisterUsage(this->rs, p) +
        this->addToRegisterUsage(R_A0, p) + this->addToRegisterUsage(R_A1, p) +
        this->addToRegisterUsage(R_A2, p) + this->addToRegisterUsage(R_A3, p) +
        this->addToRegisterUsage(R_SP, p);
    if (this->builtin)
...
public:
  Jalr(uint32_t address, int opcode, MIPS_register_t rs)
      : Jal(address, opcode, rs, 0) {}
...
public:
  JalReturn(uint32_t address, int opcode, int32_t extra)
      : Jal(address, opcode, extra) {}
...
public:
  Jr(uint32_t address, int opcode, MIPS_register_t rs)
      : BranchInstruction(address, opcode, rs, R_ZERO, R_ZERO, 0) {}
  ...
  int fillSources(int *p) { return this->addToRegisterUsage(this->rs, p); };
...
public:
  TwoRegisterConditionalJump(const char *what, uint32_t address, int opcode,
                             MIPS_register_t rs, MIPS_register_t rt,
                             int32_t extra)
      : BranchInstruction(address, opcode, rs, rt, R_ZERO, extra) {
    this->bc = what;
  ...
  bool pass1() {
    ...
    JavaMethod *dstMethod =
        controller->getMethodByAddress(dstInsn->getAddress());
...
public:
  OneRegisterConditionalJump(const char *what, uint32_t address, int opcode,
                             MIPS_register_t rs, int32_t extra)
      : BranchInstruction(address, opcode, rs, R_ZERO, R_ZERO, extra) {
    this->bc = what;
    this->dst = (this->address + 4) + (this->extra << 2);
  }
