...
Controller::Controller(const char *argv0, const char **defines,
                       const char *dstdir, const char *elf_filename, int n_dbs,
  ...
  xsnprintf(this->m_baseDir, len, "%s%s%s/%s/", isRelative ? cwd : "",
            isRelative ? "/" : "", dn, syscallRelativePath);
...
const char *Controller::getInstallDirectory() { return this->m_baseDir; }
...
void Controller::readSyscallDatabase(const char *filename) {
  ...
  args_start = (unsigned long)data + this->getSyscallFileLong(data, idx++);
  strtab = (char *)data + this->getSyscallFileLong(data, idx++);

  /* Add to the syscall directories and sets */
  this->n_syscall_dirs += n_dirs;
  this->syscall_dirs = (char **)xrealloc(
      this->syscall_dirs, this->n_syscall_dirs * sizeof(char **));
  for (int i = first_syscall_dir; i < this->n_syscall_dirs; i++)
    this->syscall_dirs[i] =
        ((char *)strtab) + this->getSyscallFileLong(data, idx++);

  this->n_syscall_sets += n_sets;
  this->syscall_sets = (char **)xrealloc(
      this->syscall_sets, this->n_syscall_sets * sizeof(char **));
  ...
  for (unsigned int i = 0; i < n; i++) {
    ...
    for (unsigned int j = 0; j < cur->nrArgs; j++) {
      ...
      cur->args[j].name = (char *)(strtab + (unsigned long)cur->args[j].name);
    }
...
void Controller::fixupExportedSymbols(cibyl_exported_symbol_t *exp_syms,
                                      size_t n) {
...
bool Controller::pass0() {
  ...
  uint32_t *text =
      (uint32_t *)textSection
          ->data; /* Would have panic:ed in the constructor otherwise */
  ...
  this->instructions =
      (Instruction **)xcalloc(n_insns + 1, sizeof(Instruction *));

  for (int i = 0; i < n_insns; i++) {
    Instruction *insn =
        InstructionFactory::getInstance()->create(textBase + i * 4, text[i]);

    /* Append to the delay slot of last instruction, if there is one */
    if (i >= 1 && last->hasDelaySlot()) {
      last->setDelayed(insn);
      insn = InstructionFactory::getInstance()->createDelaySlotNop(
          insn->getAddress());
    }
  ...
  this->callTableMethod = new CallTableMethod(n_functions + 1, exp_syms, n);
  /* Create methods */
  n = 0;
...
void Controller::allocateClasses() {
  ...
  while (last < this->n_methods) {
    ...
    this->classes =
        (JavaClass **)xrealloc(this->classes, (n + 1) * sizeof(JavaClass *));
    while (size < config->classSizeLimit && last < this->n_methods) {
    ...
    this->classes[n] = new JavaClass(buf, this->methods, first, last - 1);
  ...
  this->classes[n] = new CallTableClass(
      "CibylCallTable", (JavaMethod **)&this->callTableMethod, 0, 0);
...
Syscall *Controller::getSyscall(uint32_t value) {
  ...
  if (!this->syscalls[value]) {
    ...
    panic_if(!p,
             "No syscall %s:\n"
             "  Are all syscall databases added on the command line "
             "(cibyl-syscalls.db)?\n",
             name);

    this->syscalls[value] =
        new Syscall(p->name, p->nrArgs, p->returns ? 'I' : 'V');
...
JavaMethod *Controller::getCallTableMethod() { return this->callTableMethod; }
...
void Controller::setPackageName(const char *name) {
  ...
  panic_if(len + 2 > sizeof(this->jasmin_package_path),
           ...
           sizeof(this->jasmin_package_path));
...
Instruction *Controller::popTryStack() {
  panic_if(this->try_stack_top <= 0, "Popping off empty stack: %d\n",
           this->try_stack_top);
  ...
  for (int i = 0; i < cl->getNumberOfMethods(); i++) {
    ...
    for (int h = hilo->hi_start; h <= hilo->hi_end; h++) {
      ...
      for (int l = hilo->lo_start; l <= hilo->lo_end; l++) {
        ...
        default:
          emit->warning(
              "Warning: Unknown opcode %d in hilo pair at 0x%08x : 0x%08x\n",
...
bool Controller::pass2() {
  ...
  for (unsigned int j = 0; j < sizeof(scns) / sizeof(ElfSection *); j++) {
    if (scns[j])
      addr = this->addAlignedSection(addr, fp, scns[j]->data, scns[j]->size,
                                     scns[j]->align);
  }
  fclose(fp);
  ...
  for (int i = 0; i < this->n_classes; i++) {
    emit->setOutputFile(
        open_file_in_dir(path, this->classes[i]->getFileName(), "w"));
  ...
  syscallWrappers = new SyscallWrapperGenerator(
      this->defines, strdup(path), this->n_syscall_dirs, this->syscall_dirs,
      this->n_syscall_sets, this->syscall_sets, this->m_syscall_used_table);
...
void Controller::addColocation(const char *str) {
  ...
  this->n_colocs++;
  this->colocs = (FunctionColocation **)xrealloc(
      this->colocs, sizeof(FunctionColocation *) * this->n_colocs);
