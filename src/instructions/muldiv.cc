...
public:
  MulDiv(const char *what, uint32_t address, int opcode, MIPS_register_t rs,
  ...
  bool pass1() {
    ...
    panic_if(!my, "The mul/div instruction at 0x%x has no method\n",
             this->getAddress());
  ...
  int fillDestinations(int *p) {
    return this->addToRegisterUsage(R_LO, p) +
           this->addToRegisterUsage(R_HI, p);
  }
  ...
  size_t getMaxStackHeight() { return 6; }
...
public:
  Mult(uint32_t address, int opcode, MIPS_register_t rs, MIPS_register_t rt)
      : MulDiv("", address, opcode, rs, rt, R_ZERO, 0) {}
...
public:
  Div(uint32_t address, int opcode, MIPS_register_t rs, MIPS_register_t rt)
      : MulDiv("", address, opcode, rs, rt, R_ZERO, 0) {}
...
public:
  Mfxx(uint32_t address, int opcode, MIPS_register_t rd, MIPS_register_t src)
      : Instruction(address, opcode, src, R_ZERO, rd, 0) {
    this->src = src;
  }
  ...
  int fillDestinations(int *p) { return this->addToRegisterUsage(this->rd, p); }

  int fillSources(int *p) { return this->addToRegisterUsage(this->src, p); };
...
public:
  Mtxx(uint32_t address, int opcode, MIPS_register_t rs, MIPS_register_t dst)
      : Instruction(address, opcode, rs, R_ZERO, R_ZERO, 0) {
    this->dst = dst;
  }
  ...
  int fillSources(int *p) { return this->addToRegisterUsage(this->dst, p); };
