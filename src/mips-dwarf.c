...
unsigned int mips_abi_regsize(enum mips_abi abi) {
  switch (abi) {
...
static const Dwarf_Op loc_aggregate[] = {{.atom = DW_OP_breg2, .number = 0}};
#define nloc_aggregate 1

enum mips_arg mips_arg_size(Elf *elf, Dwarf_Die *functypedie,
  ...
  case DW_TAG_ptr_to_member_type: {
    Dwarf_Word size;
    if (dwarf_formudata(
            dwarf_attr_integrate(typedie, DW_AT_byte_size, &attr_mem), &size) !=
        0) {
    ...
    if (tag == DW_TAG_base_type) {
      Dwarf_Word encoding;
      if (dwarf_formudata(
              dwarf_attr_integrate(typedie, DW_AT_encoding, &attr_mem),
              &encoding) != 0)
        return UNKNOWN;
