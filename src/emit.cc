...
void Emit::bc_aload(int nr) { this->bc_load_store_helper("aload", nr); }

void Emit::bc_iload(int nr) { this->bc_load_store_helper("iload", nr); }

void Emit::bc_istore(int nr) { this->bc_load_store_helper("istore", nr); }

void Emit::bc_astore(MIPS_register_t reg) {
  panic_if(regalloc->regIsStatic(reg), "Astore to static reg %d is not allowed",
           (int)reg);
  panic_if(reg == R_ZERO || !regalloc->regIsAllocated(reg),
           "Astore to R_ZERO or unallocated reg %d is not allowed", (int)reg);
  this->bc_load_store_helper("astore", regalloc->regToLocal(reg));
}

void Emit::bc_ret(MIPS_register_t reg) {
  panic_if(regalloc->regIsStatic(reg), "ret to static reg %d is not allowed",
           (int)reg);
  panic_if(reg == R_ZERO || !regalloc->regIsAllocated(reg),
           "ret to R_ZERO or unallocated reg %d is not allowed", (int)reg);
...
void Emit::bc_popregister(MIPS_register_t reg) {
  if (regalloc->regIsStatic(reg))
    ...
    this->error("Warning/Error at 0x%08x: Popping to register %s, which is not "
                "allocated\n",
                controller->getCurrentInstruction()->getAddress(),
...
void Emit::bc_lookupswitch(int n, uint32_t *table, const char *def) {
  this->output("\tlookupswitch\n");

  for (int i = 0; i < n; i++) {
    this->write("\t\t0x%x : L_%x", table[i], table[i]);
  }
  this->write("\t\tdefault: %s", def);
}

void Emit::bc_tableswitch(int first, int n, uint32_t *table, const char *def) {
  this->write("\ttableswitch %d %d", first, first + n - 1);

  for (int i = 0; i < n; i++) {
    this->write("\t\tL_%x", table[i]);
  }
  this->write("\tdefault: %s", def);
}
...
void Emit::output(const char *what) { fprintf(this->fp, "%s", what); }

void Emit::setOutputFile(FILE *fp) { this->fp = fp; }

void Emit::closeOutputFile() { fclose(this->fp); }
