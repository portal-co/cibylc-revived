...
int instruction_to_string(Instruction *insn, char *buf, int buf_len) {
  ...
  case OP_SLT:
    out =
        snprintf(buf, buf_len, "%s %s, %s, %s", insn_name, mips_reg_strings[rd],
                 mips_reg_strings[rs], mips_reg_strings[rt]);
    break;
  case OP_SLL:
  case OP_SRA:
  case OP_SRL:
    out = snprintf(buf, buf_len, "%s %s, %s, %d", insn_name,
                   mips_reg_strings[rd], mips_reg_strings[rt], (int32_t)extra);
    break;
    /* Ifmt */
  case OP_SLTIU:
  case OP_SLTI:
  ...
  case OP_ORI:
    out = snprintf(buf, buf_len, "%s %s, %s, %d", insn_name,
                   mips_reg_strings[rt], mips_reg_strings[rs], (int32_t)extra);
    break;
    /* Jfmt */
  case OP_BGEZAL:
  case OP_JAL:
  ...
  case OP_SWL:
  case OP_SWR:
    out = snprintf(buf, buf_len, "%s %s, %d(%s)", insn_name,
                   mips_reg_strings[rt], extra, mips_reg_strings[rs]);
    break;
  ...
  case OP_MTHI:
    ...
    break;
  case OP_LUI:
    out = snprintf(buf, buf_len, "%s %s, 0x%x", insn_name, mips_reg_strings[rt],
                   extra << 16);
    break;
  ...
  case OP_BEQ:
  case OP_BNE:
    out = snprintf(buf, buf_len, "%s %s, %s, 0x%x", insn_name,
                   mips_reg_strings[rs], mips_reg_strings[rt],
