...
BasicBlock::BasicBlock(Instruction **insns, bb_type_t type, int first, int last)
    : CodeBlock() {
...
void BasicBlock::handleRegisterTablesPrev(Instruction **rtab,
                                          Instruction **wtab,
                                          Instruction *insn) {
  this->handleTables(rtab, wtab, insn, true);
}

void BasicBlock::handleRegisterTablesAfter(Instruction **rtab,
                                           Instruction **wtab,
                                           Instruction *insn) {
...
void BasicBlock::fillInstructionReadsAndWrites(Instruction **rtab,
  ...
  for (unsigned i = 0; i < sizeof(regs) / sizeof(mips_register_type_t); i++) {
    ...
    if (uses[reg]) {
      if (before)
        insn->setPrevRegisterReadAndWrite(rtab[reg], wtab[reg], regs[i]);
      else
        insn->setNextRegisterReadAndWrite(rtab[reg], wtab[reg], regs[i]);
    }
  }
}
...
bool BasicBlock::pass1() {
  bool out = true;
  Instruction **write_table =
      (Instruction **)xcalloc(N_REGS, sizeof(Instruction *));
  Instruction **read_table =
      (Instruction **)xcalloc(N_REGS, sizeof(Instruction *));
  ...
  for (int i = 0; i < this->n_insns; i++) {
    ...
    if (insn->getOpcode() == OP_J) {
      ...
      if (src != dst) {
        ...
        emit->warning("Jump from 0x%x to 0x%x not within the same method, "
                      "replacing with jal\n",
                      this->address, dstAddress);
        this->instructions[i] =
            InstructionFactory::getInstance()->createTailCallJump(
                insn->getAddress(), insn->getExtra());
     ...
     * slot is pushed before the delay slot */
    if (insn->hasPrefix())
      this->handleRegisterTablesPrev(read_table, write_table,
                                     insn->getPrefix());
  ...
  for (int i = this->n_insns - 1; i >= 0; i--) {
    ...
    if (insn->hasPrefix())
      this->handleRegisterTablesAfter(read_table, write_table,
                                      insn->getPrefix());
  ...
  for (int i = 0; i < this->n_insns; i++) {
    ...
    if (config->optimizePruneStackStores &&
        ((this->type == PROLOGUE && insn->getOpcode() == OP_SW &&
          insn->getRs() == R_SP) ||
      ...
      /* Loop through all caller saved registers and for writes, simply skip
       * them */
      for (int j = 0; mips_caller_saved[j] != R_ZERO; j++) {
        MIPS_register_t reg = mips_caller_saved[j];

        if ((this->type == PROLOGUE && p_s[reg] != 0 &&
             !insn->getPrevRegisterWrite(I_RT)) ||
            (this->type == EPILOGUE && p_d[reg] != 0)) {
          this->instructions[i] =
              InstructionFactory::getInstance()->createNop(insn->getAddress());
          if (insn->isBranchTarget())
            this->instructions[i]->setBranchTarget();
          delete insn;
          insn = this->instructions[i];
    ...
    if (insn->getDelayed())
      this->maxStackHeight =
          max(this->maxStackHeight, insn->getDelayed()->getMaxStackHeight());
  }
...
Instruction *BasicBlock::lookupPrevRegister(Instruction *insn,
                                            MIPS_register_t reg,
                                            bool is_write) {
...
void BasicBlock::commentInstruction(Instruction *insn) {
  ...
  if (insn->getAddress() >= config->traceRange[0] &&
      insn->getAddress() < config->traceRange[1]) {
    emit->bc_ldc(buf);
...
bool BasicBlock::pass2() {
  ...
  for (int i = 0; i < this->n_insns; i++) {
    ...
    if ((insn->isBranchTarget() ||
         controller->hasJumptabLabel(insn->getAddress())) &&
        !insn->isDelaySlotNop())
    ...
    if (insn->hasDelaySlot() &&
        controller->hasJumptabLabel(insn->getDelayed()->getAddress())) {
      uint32_t addr = insn->getDelayed()->getAddress();

      emit->warning(
          "Instruction on address 0x%x is in delay slot but has a label\n",
          addr);
