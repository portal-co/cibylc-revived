...
Function::Function(const char *name, Instruction **insns, int first_insn,
                   int last_insn)
    : CodeBlock() {
  ...
  for (int i = first_insn; i < last_insn; i++) {
    ...
    if ((insn->isBranch() || insn->isBranchTarget()) &&
      ...
      if (last != first) {
        this->bbs[n_bbs] =
            new BasicBlock(insns, type, first_insn + first, first_insn + last);
        this->size += this->bbs[n_bbs]->getSize();
        n_bbs++;
        this->bbs = (BasicBlock **)xrealloc(this->bbs,
                                            (n_bbs + 1) * sizeof(BasicBlock *));
        this->bbs[n_bbs] = NULL;
      }
      first = last;
  ...
  if (last != first) {
    this->bbs[n_bbs] =
        new BasicBlock(insns, NORMAL, first_insn + first, last_insn);
    this->size += this->bbs[n_bbs]->getSize();
    n_bbs++;
  }
  this->n_bbs = n_bbs;
...
Function::~Function() { free(this->name); }
...
StartFunction::StartFunction(const char *name, Instruction **insns,
                             int first_insn, int last_insn)
    : Function(name, insns, first_insn, last_insn) {
  free(this->name);
