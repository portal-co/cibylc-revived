...
FunctionColocation::FunctionColocation(const char *str) {
  ...
  while (*p) {
    ...
    this->n_fns++;
    this->fns =
        (Function **)xrealloc(this->fns, sizeof(Function *) * this->n_fns);
    this->fn_names = (const char **)xrealloc(
        this->fn_names, sizeof(const char *) * this->n_fns);
  ...
  panic_if(this->n_fns == 0, "Function coloc string %s has the wrong format\n",
           str);
}
...
map<const char *, FunctionColocation *, cmp_str>
    FunctionColocation::name_to_coloc;
