...
SyscallWrapperGenerator::SyscallWrapperGenerator(
    const char **defines, const char *dstdir, int n_syscall_dirs,
    char **syscall_dirs, int n_syscall_sets, char **syscall_sets,
    Controller::CibylDbTable_t &used_syscalls)
    : m_used_syscalls(used_syscalls) {
  ...
  size_t syscallDirLen =
      strlen(controller->getInstallDirectory()) + strlen(syscallPath) + 4;
  this->m_globalSyscallDirectory = (char *)xcalloc(syscallDirLen, sizeof(char));

  xsnprintf(this->m_globalSyscallDirectory, syscallDirLen, "%s/%s/",
            controller->getInstallDirectory(), syscallPath);

  this->set_usage = (int *)xcalloc(n_syscall_sets, sizeof(int));

  /* Fixup set usage */
  for (Controller::CibylDbTable_t::iterator it = this->m_used_syscalls.begin();
       it != this->m_used_syscalls.end(); ++it) {
...
void SyscallWrapperGenerator::doOneArgumentGet(cibyl_db_entry_t *p,
                                               cibyl_db_arg_t *a) {
  if (strcmp(a->javaType, "void") == 0 || strcmp(a->javaType, a->type) == 0)
    return;
  emit->generic("    %s %s = ", a->javaType, a->name);
  if (strcmp(a->javaType, "boolean") == 0)
    emit->generic("(boolean) (__%s == 0 ? false : true)", a->name);
  else if (strcmp(a->javaType, "String") == 0)
    emit->generic("CRunTime.charPtrToString(__%s)", a->name);
  else
    emit->generic("(%s)CRunTime.objectRepository[__%s]", a->javaType, a->name,
                  a->name);
  emit->generic(";\n");
}
...
void SyscallWrapperGenerator::doOneNonGenerated(const char *inDir,
  ...
  data = (const char *)read_cpp(&size, this->defines,
                                "%s/%s/implementation/%s.java", dir, p->set,
                                p->name);
...
void SyscallWrapperGenerator::doOne(cibyl_db_entry_t *p) {
  ...
  emit->generic("  public static final %s %s(", p->returns ? "int" : "void",
                p->name);
  for (unsigned int i = 0; i < p->nrArgs; i++) {
    ...
    emit->generic("int %s%s%s",
                  ...
                  strcmp(a->type, a->javaType) == 0 ? "" : "__", a->name,
                  i == p->nrArgs - 1 ? "" : ", ");
  }
  emit->generic(") %s {\n", (p->qualifier & CIBYL_DB_QUALIFIER_THROWS) != 0
                                ? "throws Exception"
                                : "");
  for (unsigned int i = 0; i < p->nrArgs; i++)
  ...
  if (p->returns)
    emit->generic("    %s ret = (%s)", p->returnType, p->returnType);
  else
    emit->generic("    ");
  ...
  if (strcmp(p->javaMethod, "new") == 0) /* Constructor */
    ...
    if (p->nrArgs != 0 && (p->args[0].flags & CIBYL_DB_ARG_OBJREF) &&
        (strcmp(p->javaClass, p->args[0].javaType) == 0)) {
      /* Object call */
      emit->generic("%s.%s(", p->args[0].name, p->javaMethod);
      n++; /* Skip the first argument */
    } else /* Static call */
      emit->generic("%s.%s(", p->javaClass, p->javaMethod);
  }

  /* Pass all arguments */
  for (; n < p->nrArgs; n++)
    emit->generic("%s%s", p->args[n].name, n == p->nrArgs - 1 ? "" : ", ");
  emit->generic(");\n");
...
void SyscallWrapperGenerator::generateImports() {
  ...
  for (int i = 0; i < this->n_syscall_dirs; i++) {
    ...
    for (int j = 0; j < this->n_syscall_sets; j++) {
      ...
      data = (const char *)read_cpp(&size, this->defines, "%s/%s/imports", dir,
                                    cur);
      if (data)
        emit->generic("%s", data);
    }
...
void SyscallWrapperGenerator::generateInits() {
  ...
  for (int i = 0; i < this->n_syscall_dirs; i++) {
    ...
    for (int j = 0; j < this->n_syscall_sets; j++) {
      ...
      data =
          (const char *)read_cpp(&size, this->defines, "%s/%s/init", dir, cur);
      if (data)
        emit->output(data);
    }
  }
  ...
  for (int i = 0; i < this->n_syscall_dirs; i++) {
    ...
    for (int j = 0; j < this->n_syscall_sets; j++) {
      ...
      while (de) {
        ...
        if (data) {
          emit->setOutputFile(
              open_file_in_dir(this->m_dstdir, de->d_name, "w"));
          emit->output(data);
...
bool SyscallWrapperGenerator::pass2() {
  ...
  for (Controller::CibylDbTable_t::iterator it = this->m_used_syscalls.begin();
    ...
    if ((p->qualifier & CIBYL_DB_QUALIFIER_NOT_GENERATED) == 0)
      ...
      this->doOneNonGenerated(this->syscall_dirs[p->user], p);
  }
  emit->generic("}\n");
  emit->closeOutputFile();
